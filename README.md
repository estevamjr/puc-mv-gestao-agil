...
#### 1. DoR da Solução Aplicado na Execução do Projeto (O LiveDoR)
O DoR da solução foi traduzido em código no middleware FastAPI por meio de esquemas rígidos do Pydantic. Na execução prática do projeto, esse DoR se transformou em uma barreira ativa de engenharia que moldou o comportamento do time. Como a gestão do backlog foi operada via chat integrado ao LLM, o desenvolvedor não conseguia abrir tarefas de forma manual ou informal no Jira. Para o backend autorizar a criação do ticket, o insumo enviado no chat precisava fornecer obrigatoriamente o contexto técnico do Tech Lead e a estrutura de BDD com cenários de sucesso (Happy Path) e erro (Exceção). Se na execução do projeto o desenvolvedor tentasse pular etapas ou omitisse os cenários de erro, o middleware rejeitava o payload, bloqueava a criação do card no Jira e exigia mais informações diretamente no chat. Assim, o DoR da solução controlou diretamente a disciplina de execução do time.

#### 2. Manifesto de Engenharia e Variabilidade do DoD Técnico
Os critérios de qualidade técnica do produto não foram engessados de forma genérica, variando conforme o risco arquitetural de cada entrega. Essa lógica foi estabelecida pelo papel de Tech Lead nos ritos de refinamento e centralizada no Épico Master de Fundações (SCRUM-5), que institui o Manifesto de Engenharia T-SONAIR. Por conta disso, os critérios técnicos nas descrições dos cards mudam conforme o componente: tarefas críticas de backend e API (como o gateway FastAPI no SCRUM-1 e SCRUM-37) trazem um DoD estrito exigindo cobertura de testes via PyTest acima de 80% e validação de schemas, enquanto tarefas de infraestrutura e conectividade sandbox (como o SCRUM-32) focam em critérios de isolamento de rede e persistência SSH.

#### 3. AI Auditor e o DoD de Processo Universal na Execução
Enquanto o DoD técnico variou por tipo de tarefa, o DoD de processo (a execução das regras de entrega da Sprint) foi universal e obrigatório para 100% dos cartões do board. Desenvolvemos um motor de automação chamado AI Auditor, conectado via webhooks à API v3 do Jira Cloud. Na execução do projeto, sempre que qualquer card tentava ser movido para a coluna concluído (Done), o robô intercepta o gatilho. Se a issue não contivesse o link da documentação oficial gerada no Confluence ou o PDF com o dossiê técnico anexado, o AI Auditor barrava o fechamento, aplicava o rollback automático do status e inseria um log de violação de compliance nos comentários (conforme registrado no histórico real dos tickets SCRUM-38, SCRUM-37 e SCRUM-33).
