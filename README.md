...
* 1. Definition of Ready (DoR) e o Conceito de LiveDoR
Diferente da agilidade passiva tradicional, onde a prontidão depende de checklists manuais, a ingestão de demandas no T-SONAIR adota o conceito de LiveDoR. A validação ocorre diretamente na camada de API do middleware backend antes que qualquer ticket seja oficialmente criado no Jira. Utilizando esquemas estritos do Pydantic, o sistema analisa o input em linguagem natural enviado via chat e exige obrigatoriamente o contexto técnico do Tech Lead e a estrutura de Critérios de Aceitação baseada em BDD, contendo ao menos um cenário de sucesso (Happy Path) e um cenário de erro (tratamento de exceção). Caso o comando falhe em fornecer essas restrições arquiteturais, a API rejeita a criação do payload por não conformidade regulatória.  
* 2. Manifesto de Engenharia e o DoD da Solução (Produto)
Os critérios de qualidade técnica variam de forma adaptativa conforme o nível de risco e a complexidade de cada componente mapeado no sequenciador da Lean Inception. Essa governança técnica foi estabelecida pelo papel de Tech Lead durante os ritos de refinamento do backlog e está centralizada sob o guarda-chuva do Épico Master de Fundações (SCRUM-5), que institui o Manifesto de Engenharia T-SONAIR. Sob essa diretriz, tarefas críticas de backend e API (como o gateway FastAPI no SCRUM-1 e SCRUM-37) possuem um DoD matemático exigindo cobertura de testes via PyTest superior a 80% e validação nativa de schemas, enquanto tarefas de infraestrutura e conectividade focam em testes de isolamento de rede e persistência.  
* 3. AI Auditor e o DoD de Processo Universal (Execução)
Para garantir o compliance contínuo da execução do projeto, o Definition of Done (DoD) de processo é universal e obrigatório para 100% dos tickets do board. Desenvolvemos um motor DevSecOps denominado AI Auditor, integrado via automações e webhooks da API v3 do Jira Cloud. Sempre que qualquer desenvolvedor tenta transicionar um card para o status de concluído (Done), o robô intercepta a requisição em milissegundos para auditar as evidências. Se o ticket não contiver o link da documentação técnica oficial gerada no Confluence ou o PDF com o dossiê de engenharia anexado, o AI Auditor executa um rollback imediato do status da issue, insere um log de violação de compliance nos comentários e dispara alertas por e-mail.
